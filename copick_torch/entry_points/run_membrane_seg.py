...
def segment_commands(func):
    ...
    options = [
        ...
        click.option("--threshold", type=float, required=False, default=0,
...
def run_segmenter(run, tomo_alg, voxel_size, session_id, threshold, gpu_id, models):
    ...
    writers.segmentation(run, predictions, 'membrain-seg', 'membranes', 
                         session_id=session_id, voxel_size=voxel_size)