...
class SplicedMixupDataset(SimpleCopickDataset):
    ...
    def _load_synthetic_zarr(self):
        ...
        try:
            ...
            # Normalize tomogram data
            self._synth_zarr_data = (self._synth_zarr_data - np.mean(self._synth_zarr_data)) / np.std(self._synth_zarr_data)
        ...
        for run in runs:
            ...
            if closest_vs:
                ...
                if tomograms:
                    ...
                    print(f"Found {len(tomograms)} tomograms in run {run.meta.name} with voxel spacing {closest_vs.meta.voxel_size}")
    ...
    def _splice_volumes(self, synthetic_region, region_mask, exp_crop):
        ...
        if self.blend_sigma > 0:
            try:
                ...
                blended = synthetic_region * weight_map + exp_crop * (1 - weight_map)
                spliced_volume = blended
            except Exception as e:
                print(f"Error during Gaussian blending: {e}")
