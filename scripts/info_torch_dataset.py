    ...
    try:
        ...
        with PdfPages(output_pdf) as pdf:
            ...
            for i, (vol, label) in enumerate(dataset):
                class_name = "background" if label == -1 else next((name for name, idx in name_to_label.items() if idx == label), f"Unknown_{label}")
        ...
        logger.error(f"Error loading or analyzing dataset: {e}", exc_info=True)
        return 1
